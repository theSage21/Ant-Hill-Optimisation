...
class Graph:
    """Creates a simple graph with some basic graph functions"""
    ...
    def get_path_length(self,path):
        ...
        Returns the sum of weights of the edges
        in the path"""
        length=0.0
        last=None
        for vertex in path:
            if last==None:
                last=vertex
                continue
            edge=frozenset((last,vertex))
            length+=self.edges[edge]
            last=vertex
        return length
    def get_shortest_path(self,a,b):
        "implementation of djikstras shortest path algorithm"
        S=set([a])
        L={}
        for v in self.nodes:
    ...
    def __generate_edges(self,connectivity):
        ...
        edges are of type
            { (v1,v2): weight,
              (v1,v2):weight,}
        function generates a graph without loops and without edges with 
        weight =0
        """
